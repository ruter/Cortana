...
# In-memory cache to prevent redundant synchronous database lookups
# during repetitive operations like add_todo, add_reminder, etc.
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    try:
        ...
        # Add to known users cache after verifying or creating
        ...
        if "duplicate" not in str(e).lower():
            ...
            # If it's a duplicate error, it means the user was inserted concurrently,
            # so we can safely add them to our cache
            _known_users.add(user_id)
