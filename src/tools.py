...
# Expected performance impact: Eliminates one synchronous DB call per repeated bot operation
_known_users = set()

def _sync_ensure_user_exists(user_id: int) -> None:
    """Synchronous helper for ensure_user_exists to offload blocking DB calls."""
    try:
        ...
        if "duplicate" not in str(e).lower():
            ...
            # If duplicate, user definitely exists
            _known_users.add(user_id)
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    - Uses in-memory _known_users set to skip redundant synchronous database lookups
    - Offloads blocking Supabase execute() calls to a thread pool via run_in_executor
    """
