...
# In-memory cache to skip repetitive database checks for known users
_known_users = set()

def _sync_ensure_user_exists(user_id: int) -> None:
    """Synchronous helper to perform database operations without blocking the event loop."""
    try:
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    # Fast path: skip database check if we already verified this user
    if user_id in _known_users:
        return

    import asyncio
    loop = asyncio.get_running_loop()
    # Run the blocking Supabase execute() in a threadpool to prevent event loop blocking
    await loop.run_in_executor(None, _sync_ensure_user_exists, user_id)

    # Mark user as known for future requests
