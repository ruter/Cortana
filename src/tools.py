...
def _check_or_create_user(user_id: int) -> None:
    """Synchronous helper to check/create user."""
    try:
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    if user_id in _known_users:
        return
    await asyncio.get_running_loop().run_in_executor(None, _check_or_create_user, user_id)
