...
def _sync_ensure_user_exists(user_id: int) -> None:
    """Synchronous helper for ensure_user_exists to be run in executor."""
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    try:
        ...
        # Add to cache only upon successful verification/creation
        _known_users.add(user_id)
    except Exception as e:
        # If error is about duplicate, it means user exists, safe to cache
