...
def _sync_ensure_user_exists(user_id: int) -> None:
    """Synchronous helper for ensure_user_exists to offload database calls."""
    try:
        ...
        # If error is not about duplicate, log it and re-raise so we don't cache
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    try:
        ...
        # Update cache only on success or expected duplicate error
