...
def _sync_check_and_create_user(user_id: int) -> bool:
    """Synchronous helper for ensure_user_exists."""
    try:
        ...
        if "duplicate" not in str(e).lower():
            ...
            return False
        return True # Duplicate means user exists
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    success = await loop.run_in_executor(None, _sync_check_and_create_user, user_id)
    if success:
