...
# Global cache to avoid redundant database calls for checking user existence
...
def _ensure_user_exists_sync(user_id: int) -> bool:
    ...
    try:
        ...
        if "duplicate" not in str(e).lower():
            ...
            return False
        # If it's a duplicate error, it means the user actually exists
        return True
...
async def ensure_user_exists(user_id: int) -> None:
    ...
    # Offload the blocking DB operations to a thread pool
